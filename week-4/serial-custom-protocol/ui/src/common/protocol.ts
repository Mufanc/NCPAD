...
export class StreamReader {
    private readonly callback: (frame: FrameInfo | null) => void
    private cache = Buffer.alloc(0)
    private timerId: number | null = null

    constructor(callback: (frame: FrameInfo | null) => void) {
    ...
    append(buffer: Buffer) {
        ...
        this.timerId = window.setTimeout(() => {
            this.callback(Protocol.decode(this.cache))
